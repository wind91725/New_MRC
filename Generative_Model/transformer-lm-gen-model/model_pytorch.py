...
class ClfHead_msmarco(nn.Module):
    """ Classifier Head for the transformer """
    def __init__(self, clf_token, cfg):
        super(ClfHead_msmarco, self).__init__()
        self.n_embd = cfg.n_embd
        self.clf_token = clf_token
        self.dropout = nn.Dropout2d(cfg.clf_pdrop) # To reproduce the noise_shape parameter of TF implementation
        self.linear = nn.Linear(cfg.n_embd, 1)
    ...
    def forward(self, h, x):
        ...
        flat = x[:, :, :, 0].contiguous().view(-1)
        clf_h = clf_h[flat == self.clf_token, :] 
        clf_h = clf_h.view(-1, 10, self.n_embd, 1)
        clf_h = self.dropout(clf_h)
        clf_h = clf_h.view(-1, self.n_embd)
        clf_logits = self.linear(clf_h)
        if self.training:
            return F.log_softmax(clf_logits.view(-1, 10), dim=1)
        else:
            return clf_logits.view(-1, 10)
...
class DoubleHeadModel(nn.Module):
    ...
    def __init__(self, cfg, clf_token, task_head_type, vocab=40990, n_ctx=512):
        ...
        if isinstance(task_head_type, str):
            ...
            elif task_head_type == 'msmarco_para_selet':
                self.task_head = ClfHead_msmarco(clf_token, cfg)
            elif task_head_type == 'msmarco_gen':
                self.task_head = None
            else:
    ...
    def forward(self, x):
        ...
        # print('lm_logits in model inside is:', lm_logits)
        return lm_logits # + self.pos_emb_mask
