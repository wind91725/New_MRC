...
def encode_dataset_msmarco(*splits, encoder):
    encoded_splits = []
    for split in splits:
        fields = []
        for idx, field in enumerate(split):
            if idx == 0:
                _field = encoder.encode(field)
            elif idx == 1:
                _field = []
                for child_field in field:
                    child_field = encoder.encode(child_field)
                    _field.append(child_field)
            elif idx == 2:
                _field = field
            field = _field
            fields.append(field)
...
def iter_data_msmarco(queries, passagess, targets, n_batch=128, truncate=False, verbose=False, max_batches=float("inf"), training=False):
    n = len(queries)
    if truncate:
        n = (n//n_batch)*n_batch
    n = min(n, max_batches*n_batch)
    ...
    for i in range(0, n, n_batch):
        if n_batches >= max_batches: raise StopIteration
        ret = []
        ret.append(queries[i:i+n_batch])
        t_passages = []
        for j in range(10):
            t_passages.append(passagess[j][i:i+n_batch])
        ret.append(t_passages)
        ret.append(targets[i:i+n_batch])
        rets.append(ret)
    if training:
        np.random.shuffle(rets)
    for ret in tqdm(rets, total=n//n_batch, file=f, ncols=80, leave=False):
        # if n_batches >= max_batches: raise StopIteration
        yield ret
        n_batches += 1
